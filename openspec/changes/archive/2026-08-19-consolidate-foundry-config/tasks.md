...
- [x] 6.2 Run the env-gated live suite from Windows on the tailnet to prove the single key authenticates both hosts — the one claim the offline suite cannot reach, since `aif-eugo-swc` has `publicNetworkAccess: Disabled`. Export the `Foundry__*` variables in a **fresh shell**: leftover `DocumentIntelligence__*` variables now fail the host at boot, and stale ones would also leave the live tests self-skipping. If the tailnet is unavailable, record it as blocked with the reason; do not weaken or skip an offline test to compensate. **BLOCKED 2026-08-18 — tailnet DNS, not the tunnel.** Re-diagnosed while connected: the tunnel is up (`RouteAll`/`CorpDNS` on), the subnet router `vm-eugo-vpn-swc` is online and advertising `10.60.0.0/16` as a primary route, TCP 443 reaches all three private endpoints (`10.60.5.4` cognitiveservices, `10.60.5.5` openai, `10.60.5.6`), and the endpoints serve a Microsoft cert whose SANs are `*.cognitiveservices.azure.com` / `*.openai.azure.com`. Only name resolution fails, and the cause is in the tailnet DNS config: the Split DNS routes for both suffixes point at **100.77.69.84, which is not a peer in the tailnet** (`tailscale ping` -> "no matching peer"); the live router is 100.111.128.91, and querying it directly also returns NXDOMAIN, so it is not running a forwarder to Azure DNS (168.63.129.16) either. Repointing Split DNS is necessary but not sufficient — the router needs a resolver. This is an EuGo-infra fix, not a docint one. Superseded note: originally recorded as "not on the tailnet". **Update, later on 2026-08-18:** the admin-console half was applied — Split DNS for both suffixes now points at the live router `100.111.128.91` — but resolution still fails, because the router runs no resolver: `Resolve-DnsName google.com -Server 100.111.128.91` returns no answer, so it is not the Foundry names that are missing but DNS service itself. The route half remains healthy (TCP 443 to `10.60.5.4` still succeeds). Closing this task needs a forwarder on `vm-eugo-vpn-swc` (or Split DNS pointed at `168.63.129.16` with a `/32` route advertised) — tracked separately as the router-DNS change, and owned by EuGo-infra — `aif-eugo-swc.cognitiveservices.azure.com` and `.openai.azure.com` do not resolve from this machine. Starting it is an infrastructure action left to the user. Everything the offline suite can reach is green; the single unproven claim is that one key authenticates both hosts, and the account exposes only one key pair (`key1`/`key2`, verified via the management plane), so no alternative outcome exists for it to have. **Re-checked 2026-08-19: still blocked, same half.** The tunnel is up and the route is healthy (TCP 443 to `10.60.5.4` succeeds), but both Foundry hostnames still return NXDOMAIN, including when queried directly at the router `100.111.128.91`, so the live suite would self-skip rather than run. Two notes above are corrected by this check: the `tailscale` CLI **is** on this workstation (`tailscale dns status` shows Split DNS correctly pointing both suffixes at the live router), and the VM is **not** deallocated — it is online, presenting as `vm-eugo-vpn-swc-1`. The rename is itself evidence for the router-DNS change's rebuild-reproducibility tasks; the regression is recorded there, in its proposal. **Diagnosis corrected 2026-08-19 — the router was never the problem.** Verified on the VM: it
resolves both Foundry hosts correctly (`10.60.5.4`, `10.60.5.5`) on the same address the
workstation queries, and cloud-init provisions it. The failure is entirely local to this
workstation: a second VPN client intercepts **every UDP port 53 query** and answers it itself, so
queries never reach the router. Proof independent of the product: a query to `192.0.2.1`, a
reserved TEST-NET address that cannot host a resolver, returns a forged NXDOMAIN, and `tcpdump` on
the router captured zero packets while queries were fired at it. Every earlier NXDOMAIN recorded
against this task was locally fabricated and proves nothing about the router.

This makes 6.2 closeable **without** waiting on EuGo-infra, by either route: disconnect the
intercepting VPN client and let Tailscale's Split DNS work, or bypass resolution altogether with
two `hosts` entries pointing the Foundry names at `10.60.5.4` / `10.60.5.5` — the route and the
certificate SANs both already check out. If the `hosts` route is used, record it as temporary:
those addresses change if the private endpoints are recreated. Either way the three `Foundry__*`
variables must be exported in a fresh shell, key included, or the live tests SKIP and the run
looks deceptively green.

**CLOSED 2026-08-19 — all 7 live tests pass against `aif-eugo-swc`: 0 failed, 0 skipped.** One key
(`key1`) authenticated both hosts in the same run, which is the claim this task existed to prove
and the one the offline suite cannot reach. OCR on the scanned PDF and the vision description on
the photo both came back from the real service.

Two things had to be fixed to get here, and only the first was known:

1. **DNS on the workstation.** Not the router — a second VPN client was intercepting every UDP 53
   query and forging NXDOMAIN. With it disconnected, both hostnames resolve to `10.60.5.4` /
   `10.60.5.5` and TLS verifies against the Microsoft certificate.
2. **The live suite could never have passed anyway.** It hosted the app through `DocIntAppFactory`,
   which blanks both Foundry endpoints on purpose to keep the offline suite hermetic. That blanking
   wins over the environment, so every live test booted an unconfigured host and failed with
   `engine_unconfigured` no matter what was exported. The suite now uses `LiveAppFactory`, which
   passes the environment's endpoints and key through before delegating to the base factory. The
   defect was invisible for as long as it existed: the tests are env-gated, so they reported
   SKIPPED rather than failing, and the endpoints were unreachable regardless.
