...
* **Statelessness:** request-scoped byte handling only; nothing written to disk or any store. **Amended 2026-08-06:** precise wording — no document is *persisted*, to disk or to any store, and nothing outlives its request. That is not the same as never writing: `SpreadsheetEngine` opens from a `MemoryStream`, but OpenXML / `System.IO.Packaging` spills package parts to a transient temp file underneath. Statelessness is intact (the file is request-scoped and discarded), but the process does need a writable temp directory — read as "never touches disk", this line led the Helm chart to ship `readOnlyRootFilesystem: true` with no writable `/tmp`, which broke every XLSX request on every deployment until 2026-08-06. Any future sandboxing decision must budget for a transient temp dir.
...
3. **chart-lint** — `helm lint` plus two `helm template` renders of `charts/eugo-docint` (added by the 2026-07-26 Helm chart work), and since 2026-08-06 an assertion that the rendered pod mounts a writable `/tmp` (the regression gate for the read-only-root/XLSX bug — see the Helm chart spec §4). Publishes nothing.
